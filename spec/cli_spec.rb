...
RSpec.describe CLI do
  ...
  let(:exchange_service) { instance_double(ExchangeRateApiServices::CurrencyRatesService) }
  let(:converter_service) { instance_double(CurrencyServices::ConvertCurrencyService) }

  describe '.start' do
    before do
      allow(exchange_service).to receive(:call)
